 ...
 * @author Andrea Wu
 */
class FilledCirc extends Picture {
    ...
    /** Constructor for FilledCirc that has to have args:
     * point (X, Y), RAD, RGB, LW.
     * Returns an instance of FilledCirc. */
    public FilledCirc(double x, double y, double rad, double[] rgb, double lw) {
        this._x = x;
        this._y = y;
        this._rad = rad;
        this._theta1 = 0;
    ...
    void draw(PrintWriter out) {
        ...
        out.println(_x + " " + _y + " " + _rad + " "
                + _theta1 + " " + _theta2 + " arc fill");
    }
    ...
    Picture move(double x, double y) {
        double a = _x + x;
        double b = _y + y;
        return new FilledCirc(a, b, _rad, _rgb, _lw);
    }
    ...
    Picture scale(double factor) {
        ...
        return new FilledCirc(a, b, rad, _rgb, _lw);
    }
}
